...
Cette instruction décrit la procédure complète pour ajouter un nouveau type de formulaire d'intervention (ex. « Contrôle vanne », « Inspection réservoir »), de la création de la table jusqu'à sa mise à disposition effective des agents et son apparition dans le tableau de bord — **sans avoir besoin de modifier le code des widgets**.
...
6. Tester avec chaque rôle avant mise en service
```
...
Dans **Règles d'accès → [nom de la nouvelle table]**, ajoutez ces règles, dans cet ordre, avant toute règle finale de type « refuser tout » :
...
### Étape 6 — Tester avant mise en service
...
4. Si l'écriture échoue avec le message *« Blocked by row create access rules »* : la règle de création (n° 2 de l'étape 5) est manquante ou mal orthographiée — ce n'est jamais un problème du widget lui-même à ce stade.
