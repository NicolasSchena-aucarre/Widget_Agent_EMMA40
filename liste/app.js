...
(function () {
    ...
    try {
      ...
      if (!technicalRow) {
        ...
        if (hasAddressField && technicalRow.Adresse) {
          html += '<div class="minimap-wrap"><div class="minimap" id="minimap"></div>' +
            '<div class="minimap-status" id="minimap-status"></div></div>';
        }
      }
  ...
  async function renderMinimap(address) {
    ...
    if (statusEl) {
      statusEl.innerHTML = '<a href="https://www.openstreetmap.org/?mlat=' + geo.lat + '&mlon=' + geo.lon +
        '#map=17/' + geo.lat + '/' + geo.lon + '" target="_blank" rel="noopener">Ouvrir en plein écran ↗</a>';
    }
  ...
  function bindModalOnce() {
    ...
    document.getElementById('modal-overlay').addEventListener('click', function (e) {
      if (e.target.id === 'modal-overlay') closeImageModal(); // clic sur le fond, pas sur l'image
    });
