...
"use strict";
  ...
  // f99r74L5RvXsoLcEZcY1BR). getAccessToken() renvoie parfois une baseUrl
  // construite avec le premier, alors que le serveur valide la requête par
  // rapport au second — d'où un 401 (masqué en erreur CORS par le
  // navigateur, faute d'en-tête CORS sur les réponses d'erreur de Grist).
  // On extrait donc le docId canonique directement depuis le jeton (il est
  // en clair dans sa partie centrale, pas besoin d'appel réseau) et on
  // corrige l'URL en conséquence.
  ...
  function buildApiBase(tokenInfo){
    ...
    if (m && canonicalDocId && m[2] !== canonicalDocId){
      ...
      console.log('[Formulaire terrain] baseUrl corrigée : ' + tokenInfo.baseUrl + ' -> ' + baseUrl);
    }
    return baseUrl;
  }
