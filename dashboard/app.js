...
(function () {
  ...
  var WIDGET_BUILD = '2026-09-08-v3';
  ...
  async function loadExcludedFields() {
    if (!hasWidgetOptionsApi()) {
      ...
      state.excludedFields = DataViz.DEFAULT_EXCLUDED_FIELDS;
      return;
    }
    try {
      var saved = await grist.widgetApi.getOption('excludedFields');
      state.excludedFields = saved ? JSON.parse(saved) : DataViz.DEFAULT_EXCLUDED_FIELDS;
    } catch (err) {
  ...
  async function openSettingsPanel() {
    ...
    try {
      var perFormulaireFields = await Promise.all(activeForms.map(function (f) {
        return DataViz.discoverChartableFields(f.Table_Technique);
      }));
  ...
  async function saveSettingsPanel() {
    var map = {};
    document.querySelectorAll('#settings-body input[type="checkbox"]').forEach(function (input) {
