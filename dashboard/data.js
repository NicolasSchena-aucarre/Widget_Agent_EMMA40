...
(function (global) {
  ...
  // Certains champs Numérique n'ont pas de moyenne qui ait un sens
  // métier (un index de compteur, un diamètre de canalisation — ce sont
  // des identifiants ou des caractéristiques techniques, pas des
  // quantités à moyenner). Contrairement au reste de la découverte
  // dynamique, ce point ne peut pas être déduit automatiquement du type
  // de colonne : c'est un jugement métier, donc une liste explicite,
  // volontairement courte, à compléter à la main si besoin.
  ...
  async function discoverChartableFields(tableId) {
    ...
    for (var i = 0; i < colsMeta.id.length; i++) {
      ...
      if (type === 'Choice') {
        ...
        var excluded = EXCLUDED_NUMERIC_FIELDS[tableId] || [];
        if (excluded.indexOf(colId) === -1) {
